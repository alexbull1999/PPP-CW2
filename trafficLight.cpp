...
TrafficLight::TrafficLight(Time delayTime, char* TName, 
		TrafficLight& partnerLight) : lightColor("red")
{
	lightDelay = delayTime;
	lightName = TName;
	lightPair = &partnerLight; //lightPair points to mem. address of the reference
	
	/* As partnerLight is passed by reference, we can access its attributes.
	 * Using this, creates a reference to the current instance of TrafficLight
	 * which we can then use to update the pairing of the first TrafficLight,
	 * from its nullptr state */
